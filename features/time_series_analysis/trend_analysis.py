...
def run_mk_cp_analysis():
    ...
    with st.spinner("📊 Processing your data..."):
        ...
        with st.container():
            ...
            with col1:
                ...
                global_mk_dict = {
                    ...
                    'Significance': "✅ Significant" if global_mk.p < 0.05 else "❌ Not Significant",
                    'Confidence Level': "95%"  # Added confidence level
                }
        ...
        if enable_cp:
            ...
            change_points = detect_change_points(signal, penalty=penalty)
            with col2:
        ...
        # Style improvements - using default style with customization
        ...
        ax.plot(df['time'], overall_trend,
                label=f"Overall Trend ({global_sen_summary['Slope (monthly)']:.2f}/month)",
        ...
        if enable_cp and len(change_points) > 1:
            colors = ['#E74C3C', '#27AE60', '#8E44AD', '#F39C12']  # Distinct colors for segments
            prev_cp = 0
            for idx, cp in enumerate(change_points):
                ...
                if analysis:
                    ...
                    label = f"Segment {idx+1}: {analysis['sen_summary']['Slope (monthly)']:.2f}/month"
                    ax.plot(segment_df['time'], analysis['trend_line'],
                           linestyle='--', linewidth=2, color=color, label=label)
                prev_cp = cp

            # Plot change points
            for cp in change_points[:-1]:
                ax.axvline(df['time'].iloc[cp], color='#34495E',
                          linestyle='--', alpha=0.5,
